...
int main() {
	...
	while (~scanf("%d %d", &k, &m)) {
		for (int i = 0; i < m; ++i)
			hashTable[i] = avlClear(hashTable[i]);
		...
		while (k--) {
			...
			if (cmd == 1) {
				...
				hashTable[number % m] = avlInsert(hashTable[number % m], number);
			}

			// remove
			else if (cmd == 2) {
				readNonNegativeInt(&number);
				hashTable[number % m] = avlRemove(hashTable[number % m], number);
			}
